 ...
 *
 * Modified by cal 2010 to remove termination key specificity
...
static char *synopsis = "Record to a file with flexible termination";

static char *descrip = 
"  RecordKey(filename.format|silence[|maxduration][|options])\n\n"
"Records from the channel into a given filename. If the file exists it will\n"
"be overwritten. Will terminate and gather keys\n"
...
"Unlike Record RecordKey will terminate on any digit and will save a message\n"
...
static int record_exec(struct ast_channel *chan, void *data)
{
	...
	if (ast_strlen_zero(data)) { /* no data implies no filename or anything is present */
		ast_log(LOG_WARNING, "Record requires two arguments (variable,filename)\n");
		return -1;
	}
