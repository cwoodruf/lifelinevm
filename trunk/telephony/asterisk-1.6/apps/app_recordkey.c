...
/*! \file
 *
 * \brief Application to record a sound file with key capture
 *
...
/*** DOCUMENTATION
	<application name="RecordKey" language="en_US">
		<synopsis>
			Record to a file with single key capture
		</synopsis>
		<syntax>
		...
		<description>
			...
			User can press any key to terminate the recording and continue and the key will be stored in <literal>${EXTEN}</literal>.
			If the user hangs up during a recording, all data will be lost and application will end only if option "d" is used.</para>
			<variablelist>
				...
				<variable name="RECORD_STATUS">
					...
					<value name="DTMF">A terminating DTMF was received (check ${EXTEN} for value)</value>
...
static int record_exec(struct ast_channel *chan, void *data)
{
	...
	if (ast_strlen_zero(data)) { /* no data implies no filename or anything is present */
		ast_log(LOG_WARNING, "RecordKey requires an argument (filename)\n");
...
AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY, "Record application with single key capture");
