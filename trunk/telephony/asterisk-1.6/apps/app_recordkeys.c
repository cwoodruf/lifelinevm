 ...
 * Modified by Cal Woodruff <cwoodruf@gmail.com>
 *
...
/*! \file
 *
 * \brief Record application with multiple key capture
 *
 * \author Matthew Fredrickson <creslin@digium.com>
 * \author Cal Woodruff (added key capture behavior) <cwoodruf@gmail.com>
 *
...
/*** DOCUMENTATION
	<application name="RecordKeys" language="en_US">
		<synopsis>
			Record to a file and capture a predetermined number of keys.
		</synopsis>
		<syntax>
			...
			<parameter name="options">
				<optionlist>
					...
					<option name="d">
						<para>Delete recording and end application if hangup detected.</para>
					</option>
					<option name="n">
					...
					<option name="p">
						<para>use '#' terminator key (DTMF) to terminate digit entry early.</para>
					</option>
					<option name="t">
						<para>use '*' terminator key (DTMF) instead of '#'.</para>
					</option>
				</optionlist>
			</parameter>
			<parameter name="numkeys">
				<para>Number of keys to capture if a key is pressed (1-255).</para>
			</parameter>
		</syntax>
		<description>
			...
			User can press the requested number of keys to terminate the recording and continue 
			and the captured keys will be stored in <literal>${EXTEN}</literal>. To allow for variable 
			length key string entry use the "p" or "t" options.
			If the user hangs up during a recording, all data will be lost and application will end 
			only if option "d" is used.</para>
			<variablelist>
				...
				<variable name="RECORD_STATUS">
					...
					<value name="DTMF">Terminating DTMFs were received (check ${EXTEN} for value)</value>
...
enum {
	...
	OPTION_QUIET = (1 << 2),
	OPTION_SKIP = (1 << 3),
	OPTION_POUND_TERMINATE = (1 << 4),
	OPTION_STAR_TERMINATE = (1 << 5),
	OPTION_DELETE = (1 << 6),
...
AST_APP_OPTIONS(app_opts,{
	...
	AST_APP_OPTION('p', OPTION_POUND_TERMINATE),
...
static int record_exec(struct ast_channel *chan, const char *data)
{
	...
	int keytimeout = 6000;		/* how long to wait for key presses */
	int keys = 0;			/* number of keys to wait for for input */
	int terminator = '\0';
	int rfmt = 0;
	int ioflags;
	int waitres;
	...
	if (ast_strlen_zero(data)) { /* no data implies no filename or anything is present */
		ast_log(LOG_WARNING, "RecordKeys requires argument (filename)\n");
	...
	if (!ast_strlen_zero(args.numkeys)) {
		sscanf(args.numkeys,"%d",&keys);
		if (keys <= 0 || keys > 255) 
			keys = 0;
	}
	...
	if (ast_test_flag(&flags, OPTION_STAR_TERMINATE))
		terminator = '*';
	if (ast_test_flag(&flags, OPTION_POUND_TERMINATE))
		terminator = '#';
			...
			if (!keys) {
				...
				break;
                        ...
                        for (i = 1; i < keys;) {
                                ...
                                tmp[i++] = res;
                                if (terminator != '\0' && tmp[i-1] == terminator) {
                                        tmp[i-1] = '\0';
                                        break;
                                }
                        }
	...
	if (!f) {
		...
		if (ast_test_flag(&flags, OPTION_DELETE)) {
...
AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY, "Record application with multiple key capture");
