...
=item $AGI->say_date($epoch, $digits)

Executes AGI Command "SAY DATE $epoch $digits"

Says the given date in epoch seconds, returning early if any of the $digits are received.

Example: $AGI->say_date(time);
...
sub say_date {
	my ($self, $epoch, $digits) = @_;

	my $ret = 0;

	$digits = '""' if (!defined($digits));

	return -1 if (!defined($epoch));
	$epoch =~ s/\D//g;
	$ret = $self->execute("SAY DATE $epoch $digits");
...
=item $AGI->say_time($epoch, $digits)

Executes AGI Command "SAY TIME $epoch $digits"

Says the given time in epoch seconds, returning early if any of the $digits are received.

Example: $AGI->say_time(time);
...
sub say_time {
	...
	return -1 if (!defined($epoch));
	$epoch =~ s/\D//g;
	$ret = $self->execute("SAY TIME $epoch $digits");
...
Example: $AGI->say_phonetic("XYZ013");
