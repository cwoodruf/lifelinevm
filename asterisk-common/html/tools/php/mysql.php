...
function ll_sync_client($box=null) {
	global $vdata;
	$personal = ll_get_client($box);
	$cid = $personal['cid'];
...
function ll_get_client($box=null) {
	global $vdata;
	...
	if (!isset($_REQUEST['personal'])) {
		if (!isset($box) && isset($_REQUEST['box'])) 
			$box = $_REQUEST['box'];
		$personal = ll_box($box);
		if (!isset($personal['cid'])) $cid = $_REQUEST['cid'];
		else $cid = $personal['cid'];
...
function ll_sync_clients($boxlist) {
	foreach ($boxlist as $bdata) {
		ll_sync_client($bdata['box']);
	}
}
